classdef AffineGridGenerator_fix < dagnn.Layer
 ...
 properties
  ...
  methods
    function outputs = forward(obj, inputs, ~)
      ...
      A = inputs{1};
      nbatch = size(A,4);
      % fix c1=0.5,c2=0,c3=0,c4=0.5
      A = reshape(A,2,1,nbatch);
      
      L = obj.fix*repmat(eye(2,2),1,1,nbatch);
    ...
    function obj = AffineGridGenerator_fix(varargin)
      obj.load(varargin) ;
